...
public class SharedTopicConnectionFactory extends ActiveMQConnectionFactory {
    ...
    // Route JMSContext creation to SharedJMSContext so the JMS 3.1 simplified
    // API (createSharedConsumer / createSharedDurableConsumer) works, rather
    // than the base ActiveMQContext whose shared-consumer methods are stubs.

    @Override
    protected JMSContext newContext(ActiveMQConnection connection) {
        return new SharedJMSContext(connection);
    }

    @Override
    protected JMSContext newContext(ActiveMQConnection connection, int sessionMode) {
        return new SharedJMSContext(connection, sessionMode);
    }
}
