...
/**
 * The original {@link SslContext} implementation that uses a {@link ThreadLocal}
 * to propagate SSL configuration through the transport factory call chain.
 *
 * <p>This class preserves full backward compatibility with the pre-interface
 * {@code SslContext} class. New code should prefer explicit parameter passing
 * and, where runtime certificate reload is needed, use a
 * {@code ReloadableSslContext} instead.
 */
public class ThreadLocalSslContext extends SslContext {
    ...
    public ThreadLocalSslContext() {
    }

    public ThreadLocalSslContext(KeyManager[] km, TrustManager[] tm, SecureRandom random) {
        if (km != null) {
    ...
    static void setCurrent(SslContext ctx) {
        current.set(ctx);
    }

    static SslContext getCurrent() {
        return current.get();
    }
