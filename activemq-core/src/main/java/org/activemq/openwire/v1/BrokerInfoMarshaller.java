...
public class BrokerInfoMarshaller extends BaseCommandMarshaller {
    ...
    public void unmarshal(OpenWireFormat wireFormat, Object o, DataInputStream dataIn, BooleanStream bs) throws IOException {
        ...
        info.setBrokerId((org.activemq.command.BrokerId) unmarsalCachedObject(wireFormat, dataIn, bs));
        info.setBrokerURL(readString(dataIn, bs));
        
		        if( bs.readBoolean() ) {
		            short size = dataIn.readShort();
		            org.activemq.command.BrokerInfo value[] = new org.activemq.command.BrokerInfo[size];
		            for( int i=0; i < size; i++ ) {
		                value[i] = (org.activemq.command.BrokerInfo)unmarsalNestedObject(wireFormat,dataIn, bs);
		            }
        ...
        info.setRedeliveryPolicy((org.activemq.command.RedeliveryPolicy) unmarsalNestedObject(wireFormat, dataIn, bs));
