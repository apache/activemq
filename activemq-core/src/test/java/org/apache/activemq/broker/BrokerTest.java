...
//                Integer.valueOf(DeliveryMode.NON_PERSISTENT), 
    ...
    public void testConsumerPrefetchAndStandardAck() throws Exception {
        ...
        // Acknowledge the first message.  This should cause the next message to get dispatched.
    ...
    public void testConsumerCloseCausesRedelivery() throws Exception {
        ...
        // Close the consumer without acking.. this should cause re-delivery of the messages.
    ...
    public void testGroupedMessagesDeliveredToOnlyOneConsumer() throws Exception {
        ...
        // All the messages should have been sent down connection 1.. just get the first 3 
        for( int i=0; i < 3 ; i++ ) {
            Message m1 = receiveMessage(connection1);
            assertNotNull("m1 is null for index: " + i, m1); 
...
//    }
    ...
    public void testTempDestinationsOnlyAllowsLocalConsumers() throws Exception {
        ...
        // Only consumers local to the temp destination should be allowed to subscribe. 
        try {
            ConsumerInfo consumerInfo2 = createConsumerInfo(sessionInfo2, destination);
            connection2.request(consumerInfo2);
            fail("Expected JMSException.");
    ...
    public void testExclusiveQueueDeliversToOnlyOneConsumer() throws Exception {
        ...
        // Second message should go to consumer 1 even though consumer 2 is ready 
        // for dispatch.
        connection1.send(createMessage(producerInfo, destination, deliveryMode));
        connection1.send(createMessage(producerInfo, destination, deliveryMode));
    ...
    public void testCompositeSend() throws Exception {
        ...
        // The messages should have been delivered to both the A and B destination.
        for( int i=0; i < 4 ; i++ ) {
            Message m1 = receiveMessage(connection1);
            Message m2 = receiveMessage(connection2);
    ...
    public void testTopicNoLocal() throws Exception {
        ...
        // The first connection should not see the initial 4 local messages sent but should 
    ...
    public void testQueueAckRemovesMessage() throws Exception {
        ...
        Message m = receiveMessage(connection);
        assertNotNull(m); assertEquals(m.getMessageId(), message1.getMessageId());
    ...
    public void testSelectorSkipsMessages() throws Exception {
        ...
        // Use selector to skip first message.
        Message m = receiveMessage(connection);
        assertNotNull(m); assertEquals(m.getMessageId(), message2.getMessageId());
        connection.send(createAck(consumerInfo, m, 1, MessageAck.STANDARD_ACK_TYPE));
