 ...
 * from the <CODE>Message</CODE> interface and adds a bytes message body. The receiver of the message supplies the
 * interpretation of the bytes. <P> The <CODE>BytesMessage</CODE> methods are based largely on those found in
 * <CODE>java.io.DataInputStream</CODE> and <CODE>java.io.DataOutputStream</CODE>. <P> This message type is for client
 * encoding of existing message formats. If possible, one of the other self-defining message types should be used
 * instead. <P> Although the JMS API allows the use of message properties with byte messages, they are typically not
 * used, since the inclusion of properties may affect the format. <P> The primitive types can be written explicitly
 * using methods for each type. They may also be written generically as objects. For instance, a call to
 * <CODE>BytesMessage.writeInt(6)</CODE> is equivalent to <CODE> BytesMessage.writeObject(new Integer(6))</CODE>. Both
 * forms are provided, because the explicit form is convenient for static programming, and the object form is needed
 * when types are not known at compile time. <P> When the message is first created, and when <CODE>clearBody</CODE> is
 * called, the body of the message is in write-only mode. After the first call to <CODE>reset</CODE> has been made, the
 * message body is in read-only mode. After a message has been sent, the client that sent it can retain and modify it
 ...
 * client. <P> If <CODE>clearBody</CODE> is called on a message in read-only mode, the message body is cleared and the
 * message is in write-only mode. <P> If a client attempts to read a message in write-only mode, a
...
public class ActiveMQBytesMessage extends ActiveMQMessage implements BytesMessage {
    ...
    private void storeContent() {
        try {
            ...
            throw new RuntimeException(ioe.getMessage(), ioe); //TODO verify RuntimeException
        }
    }
    ...
    /**
     * Reads a string that has been encoded using a modified UTF-8 format from the bytes message stream. <P> For more
     * information on the UTF-8 format, see "File System Safe UCS Transformation Format (FSS_UTF)", X/Open Preliminary
     * Specification, X/Open Company Ltd., Document Number: P316. This information also appears in ISO/IEC 10646, Annex
     * P.
     *
    ...
    /**
     * Reads a byte array from the bytes message stream. <P> If the length of array <code>value</code> is less than the
     * number of bytes remaining to be read from the stream, the array should be filled. A subsequent call reads the
     * next increment, and so on. <P> If the number of bytes remaining in the stream is less than the length of array
     * <code>value</code>, the bytes should be read into the array. The return value of the total number of bytes read
     * will be less than the length of the array, indicating that there are no more bytes left to be read from the
     * stream. The next read of the stream returns -1.
     *
    ...
    /**
     * Reads a portion of the bytes message stream. <P> If the length of array <code>value</code> is less than the
     * number of bytes remaining to be read from the stream, the array should be filled. A subsequent call reads the
     * next increment, and so on. <P> If the number of bytes remaining in the stream is less than the length of array
     * <code>value</code>, the bytes should be read into the array. The return value of the total number of bytes read
     * will be less than the length of the array, indicating that there are no more bytes left to be read from the
     ...
     * <code>value</code>, then an <code>IndexOutOfBoundsException</code> is thrown. No bytes will be read from the
     * stream for this exception case.
     *
     * @param value  the buffer into which the data is read
    ...
    /**
     * Converts the <code>float</code> argument to an <code>int</code> using the <code>floatToIntBits</code> method in
     * class <code>Float</code>, and then writes that <code>int</code> value to the bytes message stream as a 4-byte
    ...
    /**
     * Converts the <code>double</code> argument to a <code>long</code> using the <code>doubleToLongBits</code> method
     * in class <code>Double</code>, and then writes that <code>long</code> value to the bytes message stream as an
    ...
    /**
     * Writes a string to the bytes message stream using UTF-8 encoding in a machine-independent manner. <P> For more
     * information on the UTF-8 format, see "File System Safe UCS Transformation Format (FSS_UTF)", X/Open Preliminary
     * Specification, X/Open Company Ltd., Document Number: P316. This information also appears in ISO/IEC 10646, Annex
     * P.
     *
    ...
    /**
     * Writes an object to the bytes message stream. <P> This method works only for the objectified primitive object
     * types (<code>Integer</code>,<code>Double</code>, <code>Long</code> &nbsp;...), <code>String</code> objects, and
     * byte arrays.
     *
