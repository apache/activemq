...
public class SchedulerBroker extends BrokerFilter implements JobListener {
    ...
    private Object resolveDeliveryDelay(Message messageSend) {
        var deliveryTime = messageSend.getDeliveryTime();
        if (deliveryTime <= 0) {
            return null;
        }
        var delay = deliveryTime - System.currentTimeMillis();
    ...
    private boolean isDeliveryDelayEnabled(ActiveMQDestination destination) {
        ...
        var policyMap = brokerService.getDestinationPolicy();
        if (policyMap == null) {
            return true;
        }
        var entry = policyMap.getEntryFor(destination);
    ...
    private static boolean shouldSchedule(Object cronValue, Object periodValue, Object delayValue,
        ...
        var propertyDriven = cronValue != null || periodValue != null || delayValue != null;
        if (propertyDriven) {
            return jobId == null;
        }
    ...
    public void scheduledJob(String id, ByteSequence job) {
        ...
        try {
            ...
            var carriedDeliveryTime = messageSend.getProperty(DELIVERY_TIME_PROPERTY);
            if (carriedDeliveryTime != null) {
                var value = (Long) TypeConversionSupport.convert(carriedDeliveryTime, Long.class);
                if (value != null) {
