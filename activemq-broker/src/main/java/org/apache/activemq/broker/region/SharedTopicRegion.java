...
public class SharedTopicRegion extends TopicRegion {
    ...
    private void validateSelectorMatch(ConsumerInfo incoming, ConsumerInfo existing)
            throws JMSException {
        // Jakarta Messaging: a consumer may only join an active shared subscription
        // that has the same topic and selector; a different topic is a new
        // subscription attempt under a name that is already in use.
        if (existing.getDestination() != null && !existing.getDestination().equals(incoming.getDestination())) {
            throw new JMSException(
                    "Topic mismatch for shared subscription '" +
                    incoming.getSubscriptionName() +
                    "': existing='" + existing.getDestination() + "', incoming='" + incoming.getDestination() + "'",
                    ActiveMQErrorCode.TOPIC_MISMATCH);
        }
