...
public abstract class BaseDestination implements Destination {
    ...
    Predicate<Subscription> canGcConsumer = subscription -> {
        ...
        // if isGcWithOnlyWildcardConsumers() is true and this is a non-durable wildcard then we can GC.
        // An attached durable subscription never permits gc - its registration and pending messages
        // live in the destination's store, which gc destroys. Note offline durable subscriptions
        // stay attached only with keepDurableSubsActive=true (the default); brokers running with
        // keepDurableSubsActive=false forfeit this protection while the subscriber is offline.
