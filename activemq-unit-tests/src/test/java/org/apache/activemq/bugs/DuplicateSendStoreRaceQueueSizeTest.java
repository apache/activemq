...
/**
 * Reproduces queue size defect D3: the mixed async/sync duplicate
 * send race (docs/bugs/queuesize-deep-dive.md).
 *
 * Recipe:
 ...
 * empty, inflight 0, the production queue size fingerprint, with no ERROR logs.
 */
@Category(ParallelTest.class)
public class DuplicateSendStoreRaceQueueSizeTest {

    private static final Logger LOG = LoggerFactory.getLogger(DuplicateSendStoreRaceQueueSizeTest.class);
    ...
    @Test(timeout = 60_000)
    public void testAsyncSyncDuplicateSendRaceInflatesQueueSize() throws Exception {
        ...
        try(var consumeSession = connection.createSession(false, Session.AUTO_ACKNOWLEDGE);
            ...
            assertTrue("INCORRECT QUEUE SIZE (D3): queue drained (1 logical message, delivered once) " +
