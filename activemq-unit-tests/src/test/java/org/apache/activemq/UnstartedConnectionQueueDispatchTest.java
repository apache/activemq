...
public class UnstartedConnectionQueueDispatchTest {
    ...
    public void testStartedConsumerReceivesAllMessagesDespiteUnstartedCompetitor() throws Exception {
        ...
        Session unstartedSession = unstartedConnection.createSession(false, Session.AUTO_ACKNOWLEDGE);
        Queue queue = unstartedSession.createQueue("test.unstarted.dispatch");
        MessageConsumer neverStarted = unstartedSession.createConsumer(queue);
        ...
        Session session = startedConnection.createSession(false, Session.AUTO_ACKNOWLEDGE);
        MessageConsumer active = session.createConsumer(queue);
        ...
        for (int i = 0; i < MESSAGE_COUNT; i++) {
            Message received = active.receive(5000);
    ...
    public void testDeferredConsumerReceivesMessageReleasedByClosedConsumer() throws Exception {
        ...
        startedConnection.start();
        Session session = startedConnection.createSession(false, Session.AUTO_ACKNOWLEDGE);
        Queue queue = session.createQueue("test.unstarted.released");
        ...
        unstartedConnection = factory.createConnection();
        Session unstartedSession = unstartedConnection.createSession(false, Session.AUTO_ACKNOWLEDGE);
        MessageConsumer second = unstartedSession.createConsumer(queue);
    ...
    public void testDeferredConsumerReceivesAfterConnectionStart() throws Exception {
        ...
        unstartedConnection = factory.createConnection();
        Session session = unstartedConnection.createSession(false, Session.AUTO_ACKNOWLEDGE);
        Queue queue = session.createQueue("test.unstarted.recovery");
        MessageConsumer consumer = session.createConsumer(queue);

        // the async pattern: listener registered before the connection starts
        final CountDownLatch delivered = new CountDownLatch(MESSAGE_COUNT);
        consumer.setMessageListener(message -> delivered.countDown());

        startedConnection = factory.createConnection();
        startedConnection.start();
        Session producerSession = startedConnection.createSession(false, Session.AUTO_ACKNOWLEDGE);
