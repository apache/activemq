...
public class SharedSubscriptionJmsTest {
    ...
    @Test
    public void testSharedConsumerSameNameDifferentTopicRejected() throws Exception {
        connection1 = factory.createConnection();
        connection1.start();
        Session session = connection1.createSession(false, Session.AUTO_ACKNOWLEDGE);
        Topic topicA = session.createTopic("test.shared.mismatch.a");
        Topic topicB = session.createTopic("test.shared.mismatch.b");

        // Jakarta Messaging: joining an active shared subscription with the same
        // name but a different topic must fail
        MessageConsumer first = session.createSharedConsumer(topicA, "mismatchSub");
        try {
            session.createSharedConsumer(topicB, "mismatchSub");
            fail("Expected JMSException joining a shared subscription on a different topic");
        } catch (JMSException expected) {
            assertEquals(org.apache.activemq.ActiveMQErrorCode.TOPIC_MISMATCH, expected.getErrorCode());
        }
        // the original subscription is unaffected
        assertNotNull(first);
        first.close();
    }

    @Test
    public void testSharedDurableConsumerSameNameDifferentTopicRejected() throws Exception {
        connection1 = factory.createConnection();
        connection1.setClientID("mismatch-durable-client");
        connection1.start();
        Session session = connection1.createSession(false, Session.AUTO_ACKNOWLEDGE);
        Topic topicA = session.createTopic("test.shared.durable.mismatch.a");
        Topic topicB = session.createTopic("test.shared.durable.mismatch.b");

        MessageConsumer first = session.createSharedDurableConsumer(topicA, "mismatchDurSub");
        try {
            session.createSharedDurableConsumer(topicB, "mismatchDurSub");
            fail("Expected JMSException joining an active shared durable subscription on a different topic");
        } catch (JMSException expected) {
            assertEquals(org.apache.activemq.ActiveMQErrorCode.TOPIC_MISMATCH, expected.getErrorCode());
        }
        first.close();
        session.unsubscribe("mismatchDurSub");
    }
}
