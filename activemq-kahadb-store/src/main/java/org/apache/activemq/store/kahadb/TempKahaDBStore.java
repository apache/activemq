...
public class TempKahaDBStore extends TempMessageDatabase implements PersistenceAdapter, BrokerServiceAware {
    ...
    public class KahaDBMessageStore extends AbstractMessageStore {
        ...
        @Override
        public void recoverNextMessages(final int offset, final int maxReturned, final MessageRecoveryListener listener, boolean revertOrderIndex) throws Exception {
            synchronized(indexMutex) {
                pageFile.tx().execute(new Transaction.Closure<Exception>(){
                    @Override
                    public void execute(Transaction tx) throws Exception {
