 ...
 */
public class RedisJobScheduler implements JobScheduler {

    private static final Logger LOG = LoggerFactory.getLogger(RedisJobScheduler.class);
	...
	private RedisJobSchedulerStore store;

    public RedisJobScheduler(String name, RedisJobSchedulerStore store) {
        this.name = name;
		this.store = store;
    }
    ...
    public void start() throws Exception {
        if (started.compareAndSet(false, true)) {
			List<RedisJob> storedJobs = store.listJobs();
			for(RedisJob job : storedJobs) {
				doScheduleJob(job);
			}
            startDispatching();
    ...
    private void doSchedule(final String jobId, final ByteSequence payload, final String cronEntry, long delay, long period, int repeat) throws IOException {
        ...
        RedisJob newJob = new RedisJob(jobId);
        ...
        lock.writeLock().lock();
        try {
			doScheduleJob(newJob);
        } finally {
            lock.writeLock().unlock();
        }
    }

    private void doReschedule(RedisJob job, long nextExecutionTime) {
	...
	private void doScheduleJob(RedisJob job) {
		store.storeJob(job);
		long executionTime = job.getNextTime();
    ...
    private long calculateNextExecutionTime(RedisJob job, long currentTime, int repeat) throws MessageFormatException {
    ...
    private void dispatch(RedisJob job) throws IllegalStateException, IOException {
        if (canDispatch()) {
    ...
    private class ScheduledTask extends TimerTask {
        ...
        private final Map<String, RedisJob> jobs = new TreeMap<>();
         ...
         */
        public Collection<RedisJob> getAllJobs() {
         ...
         */
        public void add(RedisJob newJob) {
        ...
        public boolean remove(String jobId) {
            boolean result = jobs.remove(jobId) != null;
			return result;
        }
        ...
        public void run() {
            ...
            try {
                ...
                try {
                    ...
                    RedisJobScheduler.this.jobs.remove(executionTime);
